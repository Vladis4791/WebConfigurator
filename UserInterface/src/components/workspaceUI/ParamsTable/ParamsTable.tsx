...
const ParamsTable = () => {
	...
	const { tableNodes, currentDevice, setTableNodes, setCurrentDevice } = useWorkspace();
	...
	const columns = React.useMemo<ColumnDef<ITableNode>[]>(
		() => [
			...
			},
			{
				...
				header: () => currentDevice.DeviceType === "Logika4" ? "Оперативный" : "",
				cell: info => {
					
					return info.row.subRows.length === 0 ? <TableCheckbox {...info} /> : null;	
				}
			},
			{
				accessorKey: 'value',
				header: () => "Значение",
				cell: (info) => {
					return info.row.subRows.length === 0 ? <TableInput {...info} /> : null;			
				},
				footer: props => props.column.id,
			},
			{
	...
	useEffect(() => {
		...
		//TODO do something with this two branch of events!!!
	...
	const table = useReactTable({
		...
		updateData: (updatedNodeProps: UpdatedNodeProps) => {
			...
			console.log("here")
			setTableNodes(old => tableService.getCopyOfTableUpdatedWithNewInfo(old, updatedNodeProps))
		},
		},
		debugTable: true,
	})

	const [clickedRow, setClickedRow] = useState<Row<ITableNode> | undefined>(undefined);
	...
	return (
		<div className="ParamsTable">
			...
			<table>
				...
				<tbody>
					{table.getRowModel().rows.map(row => {
						return (
							<tr key={row.id} onClick={() => setClickedRow(row)}>
			...
			<button onClick={() => console.log(tableService.getTagsInfoFromTableData(tableNodes), tableNodes)}>submit</button>
		</div>
	)
}
